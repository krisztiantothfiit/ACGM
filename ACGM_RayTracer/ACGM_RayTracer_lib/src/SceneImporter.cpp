...
std::shared_ptr<acgm::Camera> acgm::SceneImporter::ReadCamera()
{
  ...
  camera = std::make_shared<acgm::Camera>(position, up_direction, forward_direction, z_near, z_far, fov_y_rad);
  camera->SetAspectRatio(render_options_.resolution.x / render_options_.resolution.y);
  return camera;
}
